...
public class MusicCatalogDS
{
	...
	//
	// Note: This method remove the existing Album object and then adds what
	// could be a new one (with potentially different address. An implementation
	// like this is OK for this application. But if we have relationships between
	// objects, then it may well be an incorrect solution
	//
	...
	{
		int fieldWidth = (int) Math.ceil(Math.log10(albums.size() * 2 + 11));
				// Modified
				// Previously was: Math.ceil(Math.log10(albums.size() * 2) + 1),
				// which returns value 0 when size is 0
				// 11 chosen to ensure that the field size is at least 2

		String formatter = "A%0" + fieldWidth + "d";
		// System.out.println(">>> " + formatter);

		String id;
		// repeat until get an id which is not currently in the collection
		do
		{
			id = String.format(formatter, random.nextInt(albums.size() * 2 + 10) + 1);
		}
		while (get(id) != null);
	...
	// will allow us to reconstruct the object. (Hence the name 'serialize')
	//
