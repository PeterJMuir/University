...
public class Sorter
{
   ...
   */
   ...
   public static <T extends StringKeyed > void stringRadixSort(AbstractList<T> container)  //for Space, comma and 26 + 1 characters
   {
      // Create list of Buckets
      ArrayList<ArrayList<T>> bucketList = new ArrayList<ArrayList<T>>();

      // Finding the max size of the keys
      int maxLength = 0;
      for(int i = 0; i < container.size() ; ++i)
      {
         int length = container.get(i).getStringKey().length();
         if(length > maxLength)
         {
            maxLength = length;
         }
      ...
      for(int position = maxLength -1; position >= 0; --position)
      {
         ...
         for(int i = 0; i < container.size(); ++i)
         {
            int bucketIndex = getStringValue(container.get(i).getStringKey(), position);
            bucketList.get(bucketIndex).add(container.get(i));
         }

         // Place data back into original list, but this time it's sorted a bit
         // more
         for(int i = 0, k = 0; i < bucketList.size(); ++i)
         {
            for(int j = 0; j < bucketList.get(i).size(); ++j, ++k)
            {
               container.set(k, bucketList.get(i).get(j));
            }
         }
      }
   }

   public static int getStringValue(String s, int p)
   {
      int value = 0;
      if(p >= s.length())
      {
   ...
   public static <T extends IntegerKeyed > void integerRadixSort(AbstractList<T> container)  //for Base 10
   {
      // Create list of Buckets
      ArrayList<ArrayList<T>> bucketList = new ArrayList<ArrayList<T>>();
      ...
      for(int i = 0; i < container.size() ; ++i)
      {
         int length = getNumberOfDigits(container.get(i).getIntegerKey());
         if(length > maxLength)
         {
            maxLength = length;
         }
      ...
      for(int position = 0; position < maxLength; ++position)
      {
         ...
         for(int i = 0; i < container.size(); ++i)
         {
            int bucketIndex = getDigitValue(container.get(i).getIntegerKey(), position);
            bucketList.get(bucketIndex).add(container.get(i));
         }
   ...
   public static int getDigitValue(int n, int p) //n is integer, p is significance level where 0 is least significant
   {
      //remove p least significant digits
      int truncate = (int)(n / Math.pow(10,p));

      //return the least significant value
      return truncate % 10;
   }
