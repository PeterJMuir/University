...
public class ShellSortPeopleFlamig
{
   /* Driver program
   ...
   public static void main(String[] args) throws Exception
   {
      ...
      if (args.length != 2)
      {
         System.out.println("Usage: ShellSortPeopleFlamig <input file> <output file>");
         System.exit(1);
      }
      ...
      Person.resetCounter(); 
      Sorter.ShellSort(people,0,people.size()-1,getFlamigIntervals(people.size()));
      System.out.println("There were " + Person.getCounter() +
                         " comparisons to sort " + people.size() + 
                         " people with Shell Sort (FLAMIG)");

      writeData(outputFileName, people);
   } 

   public static int[] getFlamigIntervals(int n)
   {
      int h = (5*n - 1)/11;
      int count = 1; // count the number of intervals in the sequence
      while(h > 1)
      {
         if(h < 5)
         {
